...
class FastPanelApp(ctk.CTk):
    ...
    def show_servers_tab(self):
        ...
        # Верхняя панель с кнопкой добавления сервера
    ...
    def show_domain_tab(self):
        ...
        add_domain_btn = ctk.CTkButton(
            top_panel,
            text="➕ Добавить домен(-ы)",
            command=self.show_add_domain_dialog
        )
        ...
        if not self.domains:
            ...
            for domain_info in self.domains:
                ...
                domain_label = ctk.CTkLabel(domain_frame, text=f"🌐 {domain_info['domain']}", font=ctk.CTkFont(size=14, weight="bold"))
                domain_label.pack(side="left", padx=10, pady=10)
    ...
    def show_add_domain_dialog(self):
        ...
        # Выбор сервера
        ctk.CTkLabel(dialog, text="Выберите сервер:").pack()
        server_ips = [s['ip'] for s in self.servers if s.get('ip')]
        if not server_ips:
            ctk.CTkLabel(dialog, text="Сначала добавьте сервер с IP адресом", text_color="red").pack()
            return
        ...
        # Кнопка сохранения
        save_button = ctk.CTkButton(dialog, text="Сохранить", command=lambda: self.add_domains(
            server_var.get(),
            domain_textbox.get("1.0", "end-1c"),
            dialog
        ))
    ...
    def add_domains(self, server_ip, domains_text, dialog):
        ...
        for domain in domains:
            self.domains.append({"domain": domain, "server_ip": server_ip})
    ...
    def show_result_tab(self):
        """Отображение вкладки с результатами"""
        self.clear_tab_container()
        ...
        for server in self.servers:
            if server.get("fastpanel_installed"):
                # Заглушки для ftp_login и ftp_password
                ftp_login = "user" + server["id"]
                ftp_password = "password" + server["id"]
                result_text += f"{server['ip']};{ftp_login};{ftp_password}\n"

        if not result_text:
            result_text = "Нет данных для отображения. Установите FastPanel на серверы."
